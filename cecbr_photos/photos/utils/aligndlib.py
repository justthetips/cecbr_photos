...
def computeMeanMain(args):
    align = cecbr_photos.photos.utils.openface.AlignDlib(args.dlibFacePredictor)
...
def alignMain(inputDir,outputDir,landmarks,dlibFacePredictor,size):
    cecbr_photos.photos.utils.openface.helper.mkdirP(outputDir)
    ...
    landmarkMap = {
        'outerEyesAndNose': cecbr_photos.photos.utils.openface.AlignDlib.OUTER_EYES_AND_NOSE,
        'innerEyesAndBottomLip': cecbr_photos.photos.utils.openface.AlignDlib.INNER_EYES_AND_BOTTOM_LIP
    }
    ...
    align = cecbr_photos.photos.utils.openface.AlignDlib(dlibFacePredictor)

    nFallbacks = 0
    for imgObject in imgs:
        print("=== {} ===".format(imgObject.path))
        outDir = os.path.join(outputDir, imgObject.cls)
        cecbr_photos.photos.utils.openface.helper.mkdirP(outDir)
