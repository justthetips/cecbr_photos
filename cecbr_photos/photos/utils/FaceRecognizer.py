...
class FaceRecogniser(object):
    ...
    def __init__(self, classifier):
        ...
        if self.classifier_path is not None:
            with open(self.classifier_path, 'r') as f:  # le = labels, clf = classifier
    ...
    def make_prediction(self, rgbFrame, bb):
        ...
        if landmarks == None:
            ...
            return None
        alignedFace = self.align.align(imgDim, rgbFrame, bb, landmarks=landmarks,
                                       landmarkIndices=cecbr_photos.photos.utils.openface.AlignDlib.OUTER_EYES_AND_NOSE)
    ...
    def reloadClassifier(self):
        with open(self.classifier_path, 'r') as f:  # Reloads character stream from pickle file
            (self.le, self.clf) = pickle.load(f)  # Loads labels and classifier SVM or GMM
    ...
    def trainClassifier(self):
        ...
        start = time.time()
        cecbr_photos.photos.utils.aligndlib.alignMain("training-images/", "aligned-images/", "outerEyesAndNose",
                                                      dlibFacePredictor, imgDim)
